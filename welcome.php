    ...
    <div class="collapse navbar-collapse animated fadeIn" id="bs-example-navbar-collapse-2">
      ...
      <ul id="login-dp" class="dropdown-menu">
        <li>
          ...
          <ul>
            <?php
            $result35 = $db->prepare("SELECT * FROM invoice WHERE hold='Yes'");
            $result35->execute();
            for($i=0; $row35 = $result35->fetch(); $i++){
              ?>
              <a href="sshold.php?iddd=<?php echo $row35['InvoiceNo']; ?>"><li style="list-style: none; display: inline; padding-left: 10px;">
                <button type="button" class="btn btn-warning"><?php echo $row35['InvoiceNo'] ?></button>
                </li></a>
